...
from CV.models.customCNN_v2 import CNN 
...
EPOCH_FILEPATH = f'tl_model_epoch_4.pt'
NUM_EPOCHS = 200
...
def training(train_dataloader: DataLoader, eval_dataloader: DataLoader, epoch_folder_path: str, epoch_filepath: str,  num_epochs: int):
    ...
    #using resnet as pretrained model and slight pruning
    model = mobilenet_v3_small(weights=MobileNet_V3_Small_Weights.DEFAULT)
    num_classes = 10
    model.classifier = nn.Sequential(
        nn.Linear(576, 128),  # 576 is mobilenet_v3_small's last feature dim
        nn.ReLU(),
        nn.Dropout(0.2),
    ...
    for epoch in range(num_epochs):
        ...
        with torch.inference_mode():
            for batch in eval_dataloader:
                ...
                pred_log_probs = model(inputs)  # [B, C]
                target_probs = torch.zeros_like(pred_log_probs)
                target_probs.scatter_(1, labels.long().unsqueeze(1), 1.0)
                loss = loss_function(pred_log_probs, target_probs)
...
if __name__ == "__main__":
    ...
    train_dataloader = preprocessing(isTraining=True, isNewDataAdded=True , folder="train/training", batch_size=BATCH_SIZE)
    print(">>> Processing and loading validation data ...")
    eval_dataloader = preprocessing(isTraining=False, isNewDataAdded=True , folder="train/validation", batch_size=BATCH_SIZE)
